        ...
        <div className="p-4 sm:p-6 lg:p-8 py-8 sm:py-12 lg:py-[64px] px-2 sm:px-4 lg:px-[8px]">
          <div className="w-full max-w-[960px] h-auto min-h-[120px] sm:min-h-[140px] lg:h-[160px] bg-[#EFEFEF] rounded-2xl sm:rounded-3xl flex flex-col justify-between p-4 sm:p-5 lg:p-6">
            ...
            <div className="flex-1 flex items-start">
              <Input value={inputValue} onChange={e => setInputValue(e.target.value)} placeholder="Olá, em que posso ajudar?" className="w-full border-none bg-transparent focus-visible:ring-0 focus-visible:ring-offset-0 text-[hsl(var(--smartshelf-text))] font-fustat font-light resize-none placeholder:text-[#4D4D4D] text-base sm:text-lg lg:text-2xl" style={{
              color: '#333333'
            }} />
            </div>
            ...
            <div className="flex items-center justify-between mt-4 lg:mt-0">
              ...
              <div className="flex items-center space-x-2 sm:space-x-4">
                <div className="hidden sm:flex items-center space-x-2 font-fustat font-normal" style={{ fontSize: '16px', color: '#4D4D4D' }}>
                  <span>AI Model</span>
                  <ChevronDown size={14} />
                </div>
                
                <Button size="icon" className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-[hsl(var(--smartshelf-blue))] hover:bg-[hsl(var(--smartshelf-blue))]/90" disabled={!inputValue.trim()}>
        ...
        <div className="flex flex-col pb-20">
          {notesData.map((note, index) => <div key={index}>
              <div className="w-full min-h-[200px] lg:h-[235px] bg-white p-4 lg:p-6 hover:shadow-sm transition-shadow duration-200 cursor-pointer flex flex-col">
                <div className="font-fustat font-medium mb-3 lg:mb-4" style={{ fontSize: '14px', color: '#A5A5A5' }}>{note.time}</div>
                <div className="font-fustat font-normal leading-relaxed flex-1 text-sm lg:text-base" style={{ color: '#4D4D4D' }}>
                  {note.content}
                </div>
              </div>
